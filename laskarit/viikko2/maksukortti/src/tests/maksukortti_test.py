...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_summan_lataaminen_ei_muuta_saldoa(self):
        self.kortti.lataa_rahaa(-13)
        self.assertEqual(str(self.kortti),'Kortilla on rahaa 10 euroa')

    def test_kortti_nollautuu_edullisesti(self):
        self.kortti.syo_edullisesti()
        self.kortti.syo_edullisesti()
        self.kortti.syo_edullisesti()
        # nyt kortin saldo on edullisen lounaan  verran
        self.kortti.syo_edullisesti()
        self.assertEqual(str(self.kortti),'Kortilla on rahaa 0.0 euroa')

    def test_kortti_nollautuu_maukkaasti(self):
